...
/* OPENFLAG variable */
#define OF_FAILIFEXIST      0x00
#define OF_OPENIFEXIST      0x01
#define OF_FAILIFNEXIST     0x00
#define OF_CREATEIFNEXIST   0x10

/* OPENMODE variable */
#define OM_NORMALFILENAME   0x00
#define OM_USEDOSCACHE      0x00
#define OM_READONLY         0x00
#define OM_WRITEONLY        0x01
#define OM_READWRITE        0x02
#define OM_EXCLUSIVEACC     0x10
#define OM_PUBLICACC        0x40
#define OM_PRIVATEHANDLE    0x80
 
FDATE myfdate;
FTIME myftime;
USHORT status,usBufsize;
PFILESTATUS pfstsBuf;
FILESTATUS fstsBuf;
HFILE Phandle;
USHORT ActionFlag;
/*****************************************************/
...
/*****************************************************/
/*  OS/2 Equivalent of setftime                      */
/*****************************************************/
   usBufsize=sizeof(fstsBuf);
   if (0 != (status = DosOpen( (PCH)filename,
             (PHFILE)&Phandle,
             &ActionFlag,        
             0L,
             0,
             OF_OPENIFEXIST,
             OM_PUBLICACC | OM_READWRITE,
             0L)))
     printf("Open status = %d\n",status);
   memmove(&myfdate,&lrec.last_mod_file_date,2);
   memmove(&myftime,&lrec.last_mod_file_time,2);
   pfstsBuf=&fstsBuf;
   if (0 != (status = DosQFileInfo(Phandle,
                  (USHORT)0x0001, 
                  (PFILESTATUS)pfstsBuf, 
                  usBufsize)))
     printf("Query status = %d\n",status);
   fstsBuf.fdateLastWrite=myfdate;
   fstsBuf.ftimeLastWrite=myftime;
   if (0 != (status = DosSetFileInfo(Phandle,
                  (USHORT)0x0001, 
                  (PFILESTATUS)pfstsBuf, 
                  usBufsize)))
     printf("Datetime status = %d\n",status);
   if (0 != (status = DosClose( Phandle )))
...
void cdecl main(int argc, char **argv)
{
   ...
   printf("OS/2 Conversion by Hollie Satterfield\n"); 
   printf("\n");
